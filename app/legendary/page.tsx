...
async function getData() {
  ...
  const legendaryList = pokemonList.filter((pokemon) => pokemon.isLegendary);
  return legendaryList;
}

function LegendaryPage({}: Props) {
  const [legendaryList, setLegendaryList] = useState<Pokemon[]>([]);
  useEffect(() => {
    getData()
      .then((data) => setLegendaryList(data))
      .catch((error) => console.error(error));
  }, []);
  return (
    <div className="py-5 px-3 grid grid-cols-2 gap-x-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-y-6 ">
      {legendaryList.map((pokemon) => (
        <Link
